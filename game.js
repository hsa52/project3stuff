...
document.addEventListener("keydown",function (e) {
    if(Math.abs(blob.position.x-castley.position.x)<50){
        endWin.visible=1;
        world.visible=0;
    }
    if(Math.abs(blob.position.x-mush2.position.x)<20) {
        world.visible=0;
        loseText.visible=1;
        playAgain.visible=1;
    }
    if(Math.abs(blob.position.x-mush.position.x)<20) {
        world.visible=0;
        loseText.visible=1;
        playAgain.visible=1;
    }
    if(Math.abs(blob.position.x-prin.position.x)<20) {
        world.visible=0;
        loseText.visible=1;
        playAgain.visible=1;
    }

    if(Math.abs(blob.position.x-mush3.position.x)<20) {
        world.visible=0;
        loseText.visible=1;
        playAgain.visible=1;
    }
